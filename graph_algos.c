...
Records *initRecords(Graph *graph, int startVertex)
{
  ...
  res->numTreeEdges = 0;
  res->tree = malloc(sizeof(Edge) * (graph->numVertices - 1));
...
{
  Edge *distanceEdges = malloc(sizeof(Edge) * (graph->numVertices - 1));
  Records *records = initRecords(graph, startVertex);
  // printRecords(records);
  addTreeeEdge(distanceEdges, startVertex, startVertex, startVertex, 0);
  while (!isEmpty(records->heap))
  {
    HeapNode minNode = extractMin(records->heap);
    int u = minNode.id;
...
EdgeList **getShortestPaths(Edge *distTree, int numVertices, int startVertex)
{
  ...
  for (int i = 0; i < numVertices; i++)
  {
    ...
    paths[end] = newEdgeList(newEdge(end, begin, new_dist), paths[begin]);
  }
  return paths;
}